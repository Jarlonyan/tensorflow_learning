...
class xDeepFM(object):
    def __init__(self):
        self.single_size, self.numerical_size, self.multi_size = 2, 9, 0 
        ...
        with tf.Session() as self.sess:
            ...
            for i in range(conf.epochs):
                ...
                for j in range(num_batchs):
                    ...
                    start = time.time()
                    _out, _loss, _ = self.sess.run((self.softmax_output, self.loss, self.optimizer), feed_dict=batch_dict)
                    end = time.time()
                    
                    if global_step % 10 == 0:
                        __out, __loss, __ = self.sess.run((self.softmax_output, self.loss, self.optimizer), feed_dict=self.valid_dict)
